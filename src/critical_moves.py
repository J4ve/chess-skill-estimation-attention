...
def compute_critical_moves(
    ...
    for that side, and the top ``top_k`` per side are returned. When the
    anomaly branch is off, every deviation and attention weight is zero, so
    ranking would be meaningless; return an empty list with a warning instead.
    """
